...
A. 已有数据
   → 我已有 Excel / CSV / Stata .dta 等格式的数据文件
...
> 所有数据获取后，统一保存为 **`cleaned/dataset.parquet`**（首选）或 `cleaned/dataset.csv`。若原始数据为 `.dta`，用 `pandas.read_stata()` 读取后转存，**不要求**保留 `.dta` 格式。
...
A. AI 生成 Python 构造脚本
   → 将原始数据清洗为回归就绪格式，保存为 cleaned/dataset.parquet
   依赖：pandas / numpy / openpyxl

B. 用户自行处理
   → 用户提供已构造好的 cleaned/dataset.parquet 或 .csv
```
...
- 异常值处理
- 标签添加
- 导出为 `cleaned/dataset.parquet`（同时可导出 `.csv` 备份）
...
□ 数据格式：已保存为 parquet/csv，Python 可正常 `pd.read_parquet()` 或 `pd.read_csv()`
```
